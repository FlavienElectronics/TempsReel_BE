...
void Tasks::CheckBatteryTask(void *arg)
{
    ...
    while (1)
    {
        ...
        if (rs == 1)
        {
            ...
            cout << "Send msg to mon: " << lvlbat->ToString() << endl
                 << flush;
            rt_mutex_acquire(&mutex_monitor, TM_INFINITE);
...
void Tasks::VisionTask(void *args)
{
    ...
    while (1)
    {
        ...
        if (openCameraLocal == 1)
        {
            ...
            if (cam->IsOpen() == 0)
            { // open cam non ok
                cout << "Problem in opening camera" << endl
                     << flush;
                rt_mutex_acquire(&mutex_monitor, TM_INFINITE);
                ...
                while (1)
                {
                    ...
                    else
                    {
                        ...
                        if (image != NULL)
                        {
                            ...
                            {
                                cout << "Stop find position" << endl
                                     << flush;
                            }

                            else
                            { // find position=1
                                cout << "Start find position" << endl
                                     << flush;
                                ;
                                ...
                                cout << "Sending position" << position->ToString() << endl
                                     << flush;
                                rt_mutex_acquire(&mutex_monitor, TM_INFINITE);
